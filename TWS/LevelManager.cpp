...
int** LevelManager::nextLevel(VertexArray& rVaLevel) {
	...
	// load the appropriete level from the file
	string levelToLoad;
	...
	// store the length(count) of the rows
	...
	// prep the 2nd array to hold the int values of the file
	...
	// loop through the file and store all of the values in the 2d array
	string row;
	int y = 0;
	while (inputFile >> row) {
	...
	for (int x = 0; x < m_LevelSize.x; x++) {
		for (int y = 0; y < m_LevelSize.y; y++) {
			...
			// Which tile from the sprite sheet is being used
